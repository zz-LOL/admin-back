...
<template>
  <div class="box">
      ...
      <div class="condition-box-l">
        <div class="condition-box-f">
          <div class="input-box">
            <div class="col">
              <span>经销商代码:</span>
              <el-input placeholder="请输入" size="medium" v-model="filter.dealerCode"></el-input>
            </div>
            <div class="col">
              <span>经销商简称:</span>
              <el-input placeholder="请输入" size="medium" v-model="filter.dealerShortName"></el-input>
            </div>
            <div class="col">
              <span>经销商全称:</span>
              <el-input placeholder="请输入" size="medium" v-model="filter.dealerName"></el-input>
            </div>
          </div>
          <div class="input-box">
            <div class="col">
    ...
    <div class="condition-box">
      ...
      <div class="tabs-content">
        <el-table v-loading="ajaxContent.loading" :data="columnsData" style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="dealerCode" label="经销商代码" width="180"></el-table-column>
          <el-table-column prop="dealerShortName" label="经销商简称" width="180"></el-table-column>
          <el-table-column prop="dealerName" label="经销商全称" width="180"></el-table-column>
          <el-table-column prop="department" label="部门" width="180"></el-table-column>
          <el-table-column prop="saleType" label="销售类型" width="180">
    ...
    <el-dialog
      ...
      width="30%">
      <span v-if="showDialogStatus === 1">您确认启用此经销商吗</span>
      <span v-if="showDialogStatus === 2">您确认禁用此经销商吗</span>
     ...
     <el-dialog
      ...
      width="30%">
      <span v-if="showBatchStatus === 1">您确认批量启用经销商吗</span>
      <span v-if="showBatchStatus === 2">您确认批量禁用经销商吗</span>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      activeName: 'first',
      showDialogStatus: 1,
      updateFunc: [null, dealerUpdateEnabled, dealerUpdateDisabled ],
      updateBatchFunc: [null, dealerBatchEnabled, dealerBatchDisabled ],
      statusId: null,
      showPage: false,
      queryId: null,
      isEdit: false,
  ...
  methods: {
    ...
    ajaxLists() {
      ...
      dealerList(this.filter).then(res => {
        if (res.code === 200) {
          var res2 = res.result;
